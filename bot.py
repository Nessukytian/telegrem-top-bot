...
from collect_posts import get_top_posts  # веб‐парсер
...
async def memes_cmd(message: Message):
    ...
    for chan, chat_link in channels:
        best_any, best_orig = get_top_posts(chan)

        if not best_any:
            await message.answer(f"@{chan}: за 24 ч постов не найдено.")
            continue

        # 1) Пересылаем самый залайканный (best_any)
        ...
        # 2) Если это был пересыл и есть оригинал — пересылаем best_orig
        if best_any["is_forward"] and best_orig:
...
async def open_chat_cb(callback: CallbackQuery):
    ...
    if link:
        await callback.message.answer(
            f"🔗 Вот ссылка на чат: <code>{link}</code>",
            parse_mode="HTML"
        )
